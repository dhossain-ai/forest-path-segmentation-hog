...
from src.segmentation  import (
    run_kmeans, build_label_map, smooth_label_map,
...
STRIDE      = 16
N_CLUSTERS  = 3
SMOOTH_SIZE = 5      # median filter kernel — higher = smoother regions
OUTPUT_DIR  = "output"
...
def main():
    ...
    color_features = compute_color_features(patches_bgr)

    # Combine HOG + color into one feature matrix
    ...
    print(f"[Step 3] Combined dim      : {total_dim}")
    ...
    fig.suptitle(
        ...
        f"HOG={hog_dim}  Color=6  Total={total_dim}",
    ...
    print(f"[Step 5] Smoothed cluster distribution:")
    for i in range(N_CLUSTERS):
